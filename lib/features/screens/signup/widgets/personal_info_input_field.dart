...
class PersonalInfoInputField extends StatelessWidget {
   PersonalInfoInputField({super.key});
              ...
              Row(
                ...
                children: [
                  Expanded(child: CustomDatePicker(
                    ...
                    enabledYear: context.read<TextFormBloc>().canEditField('year', 'month') && state.fields['motherName'] != null && state.fields['motherName'].isNotEmpty,
                    onChangeMonth: (value) {
                      context.read<TextFormBloc>().add(UpdateFieldEvent(field: "month", value: value));
                    },
                    enabledMonth: context.read<TextFormBloc>().canEditField('month', 'day') && state.fields['year'] != null && state.fields['year'].isNotEmpty,
                    onChangeDay: (value) {
                      context.read<TextFormBloc>().add(UpdateFieldEvent(field: "day", value: value));
                    },
                    enabledDay: context.read<TextFormBloc>().canEditField('day', 'gender') && state.fields['month'] != null && state.fields['month'].isNotEmpty,
                  )),
                  SizedBox(width: 20),
                  Expanded(child: GenderPicker(
                    ...
                    enabled: context.read<TextFormBloc>().canEditField('gender', 'address') && state.fields['day'] != null && state.fields['day'].isNotEmpty,
                  )),
                ],
              ),
            ],
